...
public class MemorySimulation2 {
    ...
    public MemorySimulation2(int pageSize, int memorySize) {
    ...
    private void readMemory(int virtualAddress) {
        ...
        int physicalAddress = (memory[pageNumber] * pageSize) + offset;
        System.out.println("读取内存地址：" + physicalAddress);
    }

    private void handlePageFault(int pageNumber) {
        pageTable[pageNumber] = true;
        memory[pageNumber] = generatePhysicalAddress();
        pageFaultCount++;
    }

    private int generatePhysicalAddress() {
        // 在实际情况中，这里可能包含页面置换算法的实现
        // 这里简单地生成一个随机的物理页框号
        Random random = new Random();
        return random.nextInt(memorySize / pageSize);
    }
